...
class NullMacro {
	...
	private static function parserNullFunc(expr:Expr):Expr {
		...
		var startKey = MacroStringTools.toFieldExpr([keys.shift()]);
		var func:Expr = macro {
			if (!NullMacro.NullTools.hasNull($startKey, $v{keys})) {
				$expr;
			}
		};
		var s = macro $v{keyscode};
		return {
			expr: func.expr,
...
class NullTools {
	public static function hasNull(value:Dynamic, keys:Array<String>):Bool {
		for (s in keys) {
